...
const Header = ({ cartItems, onCartClick }) => {
  ...
  const [isSpecialOffersMenuOpen, setIsSpecialOffersMenuOpen] = useState(false);
  ...
  // Add function to handle special offers menu
  const toggleSpecialOffersMenu = () => {
    setIsSpecialOffersMenuOpen(!isSpecialOffersMenuOpen);
    // When opening the menu, add a class to prevent body scrolling
    if (!isSpecialOffersMenuOpen) {
      document.body.classList.add('menu-open');
    } else {
      document.body.classList.remove('menu-open');
    }
  ...
  // Close special offers menu when route changes
  useEffect(() => {
    if (isSpecialOffersMenuOpen) {
      setIsSpecialOffersMenuOpen(false);
      document.body.classList.remove('menu-open');
    }
  }, [location.pathname]);
  ...
  return (
    <>
      ...
      >
          ...
          <nav className="hidden md:flex space-x-4">
            {navigationItems.map((item, index) => (
              <NavLink 
                ...
                to={item.path} 
                label={item.name} 
                isNested={item.subcategories.length > 0}
                navItem={item}
                onClick={item.name === "پیشنهادات ویژه" ? (e) => {
                  e.preventDefault();
                  toggleSpecialOffersMenu();
                } : undefined}
              />
            ))}
          </nav>
        ...
        {/* Special Offers Banner - Add this new section */}
        <AnimatePresence>
          {isScrolled && (
            <motion.div
              ...
              className="w-full bg-gradient-to-r from-draugr-900 via-draugr-800 to-draugr-900 overflow-hidden border-t border-draugr-700/30"
            >
              <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="animate-pulse">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-draugr-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" />
                    </svg>
                  </div>
                  <span className="text-sm font-bold text-white">پیشنهادات ویژه</span>
                  <span className="hidden sm:inline text-sm text-draugr-300">|</span>
                  <span className="hidden sm:inline text-xs text-draugr-300">تخفیف‌های استثنایی محصولات محدود</span>
                </div>
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  onClick={toggleSpecialOffersMenu}
                  className="text-xs bg-draugr-700 hover:bg-draugr-600 text-white px-3 py-1 rounded-md transition-colors duration-300 flex items-center gap-1"
                >
                  <span>مشاهده</span>
      ...
      {/* Special Offers Menu */}
      <SpecialOffersMenu 
        isOpen={isSpecialOffersMenuOpen} 
        onClose={() => {
          setIsSpecialOffersMenuOpen(false);
          document.body.classList.remove('menu-open');
        }} 
...
const NavLink = ({ to, label, isNested = false, navItem, onClick }) => {
